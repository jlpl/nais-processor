...
def nais_processer(path='../data',
    ...
    for i in range(number_of_data_days-1,number_of_data_days):
        # Read ion data
        ions = pd.read_table(path+'/'+ion_filenames[i],
                             sep=delimiter,
                             comment='#',
                             engine='python')
        ...
        if records_measured:
            ...
            records = pd.read_table(path+'/'+records_filenames[i],
                                    sep=delimiter,
                                    comment='#',
                                    engine='python')
...
#            flow_name_neg='neg_sampleflow.mean'
        ...
        np.savetxt(path+'/processed_data/'+ion_filenames[i]\
        ...
        if particles_measured:
            particles = pd.read_table(path+'/'+particle_filenames[i],
                                 sep=delimiter,
                                 comment='#',
                                 engine='python')
            ...
            np.savetxt(path+'/processed_data/'+particle_filenames[i]
...
def nais_plotter(path='../exmpl_data/processed_data'):
   ...
   # Take the lastest files and make a plot
   for i in range(num_files-1,num_files):
       if particles_measured:
           ...
           fig.suptitle('Marambio, '+date_str, y=1)
           
           plt.savefig(path+'/figures/'+'fig_'+date_str,
